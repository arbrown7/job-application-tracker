...
import { getAllJobStatuses, getAllJobTypes } from '../../models/jobs/jobs.js';
/**
...
/**
 * Validation rules for editing a job
 */
const jobEditValidation = [
    ...
    body('jobStatus')
        ...
        .custom( async (value) => {
            ...
            const validStatusIds = statuses.map(status => String(status.id));

            if (!validStatusIds.includes(String(value))) {
                throw new Error('Selected status type does not exist.');
            }
